...
此包是传输适配器，而非 UI 集成或通用能力 seam。部署可以选择启用标准 ACP 会话模型选择器，其数据来自适配器拥有的模型目录。桥接层不公开编辑器导航、transcript（文本记录）回放、命令、模式、其他配置选择器、信息征集、推理（reasoning）、计划、标题或工具展示。交互式渲染与向用户提问属于 Web 宿主和客户端模块。
...
| `modelSelection` | `false` | 启动时读取已配置提供方的模型目录，并通过标准 ACP 暴露为会话局部模型选择器。 |
| `modelReasoningDefaults` | `{}` | 模型 ID 到适配器原生推理强度的映射，与会话模型选择一同应用。非空时要求启用 `modelSelection`。 |

模型选择关闭时，`provider` 与 `model` 仍可省略，以便由另一个 agent/request 监听器提供目标。启用 `modelSelection` 时必须同时提供这两个字段；适配器目录必须非空且包含已配置模型，否则启动会明确失败。

每项推理默认值必须指向目录内模型，以及该确切适配器路由声明的强度；非法配置会在接受会话前使启动失败。选择时一同替换提供方、模型和已配置推理强度。省略的映射项会移除会话原有覆盖值，沿用适配器默认行为。各模型默认值不同的部署不得再强制设置一个提供方级推理强度。本配置不增加强度选择器。

## 协议约定

| 方法 | 行为 |
|---|---|
| `initialize` | 协商受支持的版本。只有挂载持久附件存储，且每个可选模型（或固定配置路由）解析后都明确支持图片输入时，才公布图片提示词能力；音频与嵌入上下文保持 false。不公布会话、编辑器、终端、文件系统或 MCP 能力。 |
| `authenticate` | 空操作，因为服务器不公布身份验证方法。 |
| `session/new` | 以绝对路径作为主 `cwd` 创建新 agent；接受空的 `additionalDirectories` 和 `mcpServers`，拒绝非空值。启用模型选择时，返回一个标准 `model` 配置项，其值保留适配器目录的顺序和元数据。 |
| `session/set_config_option` | 启用模型选择时，只一同改变指定空闲会话的模型与已配置推理默认值，并返回完整的当前配置项集合。未知配置项、目录外模型、布尔值以及提示词执行期间的变更都会被拒绝，且不修改状态。 |
...
一个连接可以拥有多个会话。桥接层以带品牌的会话 id 作为记录键，并在路由事件或权限请求前检查 agent 是否为同一对象。每个会话都有独立的模型选择、提示词槽位、工作区、取消路径和资源释放器。选择通过 Agent 作用域内的模型路由原语安装，因此提示词变量和请求路由会观察同一个逐步骤快照。
