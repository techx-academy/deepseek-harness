...
> 目录决策仍然有效。原先宽泛的 ACP 编辑器界面已由 [ACP 作为仅面向自动化的协议](../simplification/2026-07-23-acp-automation-only-protocol.md)取代；随后，[标准 ACP 会话模型选择](2026-08-19-acp-session-model-selection.md)恢复了一个窄而显式启用的标准模型选择器。
...
ACP 自动化传输层最初不消费目录。其部署配置只为新创建的 agent 提供一个可选的提供方／模型目标。[标准 ACP 会话模型选择](2026-08-19-acp-session-model-selection.md)随后增加了显式启用能力：通过标准 ACP 配置接口投影该提供方由适配器拥有的确切目录，同时默认继续保留固定路由行为。
...
- 面向人类的目录消费方拥有各自的交互。ACP 默认使用固定部署目标，也可以显式地通过标准会话模型配置项投影同一提供方的目录。
- 请求头与基于提供方路由的会话形态保持兼容；不需要新的 JSONL 事件或格式版本。
- 目录读取可以是异步的，且每个调用方都会收到值的独立副本。

## 测试

单元测试覆盖目录值副本与格式错误的元数据、pi-ai 和 DeepSeek 目录投影、提供方／模型请求路由，以及提示词变量对齐；监听器安装在 agent 作用域的上下文中，因此能够实现 agent 间隔离。ACP 传输测试同时验证固定提供方／模型转发和显式标准选择器；TUI 套件覆盖自身的选择器交互与基于请求头的恢复。
