...
桥接层只发出已提交的 `assistant/message` 文本与图片。每个会话使用一条 Promise 链，在异步重新读取并校验助手图片引用、将其转换为 ACP base64 交付时保持块与消息顺序；对象缺失或损坏会使提示词交付失败，而不是变成占位符。推理、原始分片、工具活动、待办事项、计划、标题、重试标记、终端元数据、diff、位置和资源链接仍保留在持久会话日志或 UI 专用传输层中。它不提供会话加载、列出与删除、命令、模式、通用配置界面、plan 评审或面向人类的询问。[标准 ACP 会话模型选择](../architecture/2026-08-19-acp-session-model-selection.md)随后通过 ACP 标准会话配置方法增加了一个窄而显式启用的模型选项，并未恢复已删除的编辑器界面。
